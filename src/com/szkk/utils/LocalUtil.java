...
public class LocalUtil {
	...
	public static LocalUtil instance = null;
	...
	public void select(Spinner spin, int arry) {
		String[] data = shareActivity.getResources().getStringArray(arry);
		List<CharSequence> list = new ArrayList<CharSequence>();
		for (int i = 0; i < data.length; ++i) {
			list.add(data[i]);
		}
		ArrayAdapter<CharSequence> adapter = new ArrayAdapter<CharSequence>(
				shareActivity, R.layout.spinner_item, R.id.tview_spinner_item,
				list);
		spin.setAdapter(adapter);
	}

	public void initPCCSpinners(final Spinner province_spinner,
			final Spinner city_spinner, final Spinner county_spinner) {
		province_spinner
				.setOnItemSelectedListener(new OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> arg0, View arg1,
						...
						if (true) {
							// county_spinner = (Spinner)
							// findViewById(R.id.county_spinner);
							// city_spinner = (Spinner)
							// findViewById(R.id.city_spinner);
	...
	public static DialogCommon showTipDialog(int id) {
		DialogCommon dialog = new DialogCommon(shareActivity,
				R.style.alert_dialog, shareActivity.getResources()
						.getString(id));
		dialog.initTipDialog();
		dialog.setCancelable(true);
		dialog.show();
		return dialog;
	}

	public static DialogCommon showTipDialog(String content) {
		DialogCommon dialog = new DialogCommon(shareActivity,
				R.style.alert_dialog, content);
		dialog.initTipDialog();
		dialog.setCancelable(true);
		dialog.show();
		return dialog;
	...
	public static DialogCommon showLoadingDialog(int id) {
		DialogCommon dialog = new DialogCommon(shareActivity,
				R.style.alert_dialog, shareActivity.getResources()
						.getString(id));
		dialog.initLoadingDialog();
		dialog.btn.setVisibility(View.GONE);
		dialog.btn.setEnabled(false);
		dialog.setCancelable(false);
		dialog.show();
		return dialog;
	}

	public static void showConfirmDialog(int id, DialogDelegate del) {
		Log.i("tag1", "shared: " + shareActivity);
		DialogCommon dialog = new DialogCommon(shareActivity,
				R.style.alert_dialog, shareActivity.getResources()
						.getString(id));
		dialog.setDelegate(del);
		dialog.initConfirmDialog();
		dialog.show();
	}
}
